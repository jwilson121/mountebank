...
/**
 * Represents the ldap proxy implementation
 * @module
 */
...
function create (logger, encoding) {
    ...
    function connectionInfoFor (proxyDestination) {
        if (typeof proxyDestination === 'string') {
            ...
            if (parts.protocol !== 'ldap:') {
                throw errors.InvalidProxyError('Unable to proxy to any protocol other than ldap',
    ...
    /**
     * Proxies a ldap request to the destination
     * @param {string} proxyDestination - The URL to proxy to (e.g. ldap://127.0.0.1:3535)
     * @param {Object} originalRequest - The ldap request to forward
